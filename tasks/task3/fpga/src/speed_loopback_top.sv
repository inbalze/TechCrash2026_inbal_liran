...
// speed_loopback_top.sv — Top-level module, 4.16 Mbps variant
//
// Identical to the baseline module EXCEPT the UART TX/RX baud
// rate is changed from 9600 to 4,166,666 bps (50 MHz ÷ 12).
// All LFSR, timer, state machine, comparator, and display logic
// is preserved byte-for-byte from the baseline.
//
// Baud rate derivation:
//   CLK = 50 MHz
//   BIT_PERIOD = 12 clock cycles
//   BAUD = 50,000,000 / 12 = 4,166,666 bps
//
// Wiring:
//   ARDUINO_IO[0] = FPGA RX ← ESP32 GPIO32 (TX)
//   ARDUINO_IO[1] = FPGA TX → ESP32 GPIO33 (RX)
//
// Fixed count: 10,000 bytes
// SW[9]   debug mode: in DONE, show expected/received checksums
// KEY[0]  start / restart
// KEY[1]  reset (active low)
// HEX5-0  show timer_ms (hex) in DONE, progress during send
...
module speed_loopback_top(
    ...
    // ---- UART TX (FPGA -> ESP32 on ARDUINO_IO[1]) ----
    // Baud rate: 4,166,666 bps (50 MHz / 12)
    reg        tx_start;
    reg  [7:0] tx_data;
    wire       tx_busy;
    wire       tx_out;
    ...
    // ---- Arduino Header IO ----
    assign ARDUINO_IO[0]    = 1'bz;         // RX input  (tri-state)
